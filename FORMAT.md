...
*Source: `CONCEPT-full-architecture.md §5` (WP-483 Ф2). Solo-authored 2026-07-14 after the intended peer-review session failed at the infrastructure level (Kimi adapter returned empty output twice, unrelated to this content). An independent cold-context review found and fixed two contradictions with the source concept (quarantine wrongly encoded as a `type` value instead of out-of-axis; an internal contradiction on whether freshness is mandatory). Three items remain flagged in-text as this document's own proposal, not a confirmed convention: `homes.yaml` glob-specificity precedence, and the quarantine `reason` enum + field names. (`ttl_days` was in this list too until 2026-07-16, when WP-476's own Ф4а reference application — see §Versioning above — confirmed it as the wire field name.)*
