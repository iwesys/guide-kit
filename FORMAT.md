...
**Platform pull** (`generator/personal_export.py`). When `personal_export: on` (default) and the user has a subscription token in `GUIDE_KIT_PLATFORM_TOKEN`, the adapter calls `personal_export.py` automatically before each guide generation. It fetches the platform-derived RCS profile and qualification stage via JSON-RPC 2.0 (`dt_read_digital_twin` / `dt_describe_by_path`, read-only allowlist) and writes a `profile.platform.yaml` alongside the user's `profile.yaml`. The adapter then merges this overlay into the declared profile on a per-field basis — priority: `manual` (declared) > `computed_from_events` (platform-derived) > `diagnostic_session` (declared). The overlay never deletes a declared key. The qualification stage arrives as an atomic bundle (`rcs.stage_derived` + `provenance.stage_label`); the overlay writes both only when parsing succeeds, otherwise it writes only `provenance.stage_label_raw` and leaves the declared stage untouched. A missing overlay file is not an error — the generator proceeds with the declared profile alone (same cold-start path as a missing `profile.yaml`).
